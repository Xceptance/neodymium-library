...
public class AllureCucumber3Jvm implements Formatter
{
    ...
    private void handleTestCaseStarted(final TestCaseStarted event)
    {
        ...
        final ScenarioDefinition scenarioDefinition = cucumberSourceUtils.getScenarioDefinition(currentFeatureFile, currentTestCase.getLine());
    ...
    private List<Parameter> getExamplesAsParameters(final ScenarioOutline scenarioOutline) {
        for (Iterator<Examples> iterator = scenarioOutline.getExamples().iterator(); iterator.hasNext();) {
            final Examples examples = iterator.next();
    ...
    private void handleHookStep(final TestStepFinished event)
    {
        ...
        if (!Status.PASSED.equals(translateTestCaseStatus(event.result)))
        {
            ...
            if (hookTestStep.getHookType() == HookType.Before)
            {
                ...
                lifecycle.updateTestCase(getTestCaseUuid(currentTestCase), scenarioResult -> scenarioResult.setStatus(Status.SKIPPED)
    ...
    private void handlePickleStep(final TestStepFinished event)
    {
        ...
        {
            statusDetails = getStatusDetails(new PendingException("TODO: implement me"))
                                                                                        .orElse(new StatusDetails());
            lifecycle.updateTestCase(getTestCaseUuid(currentTestCase), scenarioResult -> scenarioResult
                                                                                                       .setStatus(translateTestCaseStatus(event.result))
                                                                                                       .setStatusDetails(statusDetails));
        }
        else
        {
            statusDetails = getStatusDetails(event.result.getError())
                                                                     .orElse(new StatusDetails());
        }
        ...
        lifecycle.updateStep(getStepUuid(event.testStep), stepResult -> stepResult.setStatus(translateTestCaseStatus(event.result)));
