...
public class JavaScript
{
    ...
    public static void waitForReady()
    {
        ...
        final Function<WebDriver, Boolean> loadingAnimation = driver -> {
            return !$(Context.configuration.javascriptLoadingAnimationSelector()).is(exist);
        };
    ...
    {
        final long timeout = Context.configuration.javaScriptTimeout();
        ...
        for (final Function<WebDriver, Boolean> condition : conditions)
        {
            ...
            while (!endEarly && System.currentTimeMillis() - start < timeout)
            {
                try
                {
                    ...
                    if (result)
                    {
                        if (Context.configuration.javaScriptMustHaveBeenActive() && wasActive)
                        {
                            endEarly = true;
                            continue;
                        }
                        else if (Context.configuration.javaScriptMustHaveBeenActive() == false)
                        {
                            endEarly = true;
                            continue;
                        }
                ...
                sleep(Context.configuration.javaScriptPoolInterval());
