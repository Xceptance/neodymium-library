    ...
    protected static WebDriver createWebdriver(final BrowserConfiguration config) throws MalformedURLException
    {
        ...
        if (StringUtils.isEmpty(testEnvironment) || "local".equalsIgnoreCase(testEnvironment))
        {
            ...
            if (chromeBrowsers.contains(browserName))
            {
                ...
                // capabilities and one for experimental capabilities). The experimental options are held internal
                // within a map of the capability map and are accessible with key "goog:chromeOptions" (constant
                // ChromeOptions.CAPABILITY). So all we have to do is to copy the keys and values of that special map
                // and set it as experimental option inside ChromeOptions.
